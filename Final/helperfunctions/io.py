...
def read_fits_headers(fits_paths: List[Path]) -> List[fits.header.Header]:
...
def max_image_size(headers: List[fits.header.Header]) -> int:
    """Determine the size of the largest image given a list of fits headers.

    Parameters
    ----------
    headers: list
        A list of .fits headers
    ...
    for i, header in enumerate(headers):
        im_shape = [header[f"NAXIS{i}"] for i in range(1, header["NAXIS"] + 1)]
        nr_pixels = np.prod(im_shape)
        bytes_per_pixel = abs(header["BITPIX"]) // 8
        im_size = nr_pixels * bytes_per_pixel
        if im_size > max_image_size:
            max_image_size = im_size
...
def headers_by_time(
    ...
    """
    datetimes = np.empty(len(headers), dtype="datetime64[ms]")
    for i, header in enumerate(headers):
...
def export_to_database(db_kwargs, im_id, row, im_meta, extracted_sources):
